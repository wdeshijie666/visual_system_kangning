...
Wait mutex → state==kRequestPosted
  → state=kBusy
  → PrepareAlgoInputFromShm()  或  PrepareAlgoInputFromPaths()   [algo_input_converter.cpp，转换逻辑预留]
  → FillMockLogs / FillPipelineSimulationLogs   [algo_result_io.h]
  → state=kDone
```
...
| `algo_input_converter.cpp` | 算法输入转换 **TODO 桩** |
...
sequenceDiagram
    ...
    SHM->>Algo: state=RequestPosted
    Algo->>Algo: PrepareAlgoInputFromPaths
    Algo->>SHM: state=Done
    SHM-->>SE: AlgoResponse
...
1. [ ] `VisualSystem.exe` 启动，`AppContext::Load` 成功  
